...
function ViewModel(params) {
    ...
    self.select = function () {
        ...
        self.output = this;
        self.trigger.call(this, 'selected-card-answer');
    };
...
ViewModel.prototype.id = 'list-card-answers';
...
ViewModel.prototype._allComputed = function () {
    if (!this.context.vms['details-card-question'] || this.context.vms['details-card-question'].status() !== 'computed') {
        return false;
    }
    return true;
};
...
ViewModel.prototype._firstNotReady = function () {
    if (!this.context.vms['details-card-question']) {
        return Promise.reject();
    }
    if (this.context.vms['details-card-question'].status() !== 'computed') {
        return this.context.vms['details-card-question'].waitForStatusChange();
    }
...
ViewModel.prototype._propagate = function () {
    ...
    this._propagating = this._waitForDependencies().then(function () {
        self.filters['question'] = self.filters['question'] || (
            self.context.vms['details-card-question'].output &&
            self.context.vms['details-card-question'].output['id']);
        self.status('ready');
...
exports.register = function () {
    ko.components.register('c-list-card-answers', {
        viewModel: {
